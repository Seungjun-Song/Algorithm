...
public class Main {
    ...
    public static void main(String[] args) throws Exception{
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < M; j++) {
                map[i][j] = Integer.parseInt(st.nextToken()) * -1;    // 바다: 0, 섬: -1
            }
        }
    ...
    static void hr() {
        for (int i=0; i<N; i++) {    // 행으로 내려오면서
            ...
            for (int j = 0; j < M; j++) {
                ...
                } else if (v1 != 0 && curr == 0) {    // 섬에서 시작했고 아직 바다
                    cost++;
                }
    ...
    static void dfs(int y, int x, int num) {
        ...
        // 4방 탐색으로 갈 수 있는 곳을 계속 가면서 재귀호출 DFS 이어간다.
        for (int d=0; d<4; d++) {
            int ny = y + dy[d];
            int nx = x + dx[d];
